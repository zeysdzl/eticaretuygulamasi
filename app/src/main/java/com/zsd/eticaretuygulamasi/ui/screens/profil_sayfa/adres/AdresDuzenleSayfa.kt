...
@Composable
fun AdresDuzenleSayfa(
    navController: NavController,
    viewModel: AdresDuzenleViewModel = hiltViewModel()
) {
    val adresState by viewModel.adres.collectAsState()
    var adresBaslik by remember { mutableStateOf("") }
    var adresSatiri by remember { mutableStateOf("") }
    val context = LocalContext.current


    LaunchedEffect(adresState) {
        adresState?.let {
            adresBaslik = it.baslik
            adresSatiri = it.adresSatiri
        }
    }
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Adresi Düzenle", fontWeight = FontWeight.Bold) },
                navigationIcon = {
                ...
                Button(
                    onClick = {
                        if (adresBaslik.isNotBlank() && adresSatiri.isNotBlank()) {
                            val guncellenmisAdres = adresState!!.copy(
                                baslik = adresBaslik,
                                adresSatiri = adresSatiri
                            )
                            viewModel.updateAdres(guncellenmisAdres)
                            Toast.makeText(context, "Adres güncellendi!", Toast.LENGTH_SHORT).show()
